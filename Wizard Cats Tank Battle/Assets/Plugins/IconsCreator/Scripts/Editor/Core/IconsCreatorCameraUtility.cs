...
namespace IconsCreationTool.Editor.Core
{
    public class IconsCreatorCameraUtility
    {
        ...
        public void SetData(GameObject targetObject, int size, float padding, float cameraDistance, float offsetX, float offsetY)
        {
            if (size < 1)
            {
        ...
        {
            _camera.transform.rotation = Quaternion.Euler(_offsetX, _offsetY, 0f);
        }
